...
const ReviewCard = memo(({ review, handleViewDish }) => (
  <div className="bg-gradient-to-t from-55% from-white to-orange-700 rounded-2xl shadow-md hover:shadow-xl transition p-6 flex flex-col items-center text-center">
    <LazyLoadImage
    ...
    <p className="text-md text-orange-700 font-bold mb-2">{review.dish}</p>
    ...
    <div className="flex justify-center mb-3">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          size={18}
          className={`${i < review.rating ? "fill-orange-700 text-orange-700" : "text-gray-300"}`}
        />
      ))}
    </div>
...
export default function Reviews() {
  ...
  const reviews = useMemo(() => [
    ...
    },
    {
      ...
      dishID: 52776,
      rating: 4,
      comment: "The Recipe was sweet , nice, and very easy ",
    },
    {
      id: 3,
      userName: "John Smith",
  ...
  return (
    <div className="min-h-screen my-20 py-12 px-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {reviews.map((review) => (
          <ReviewCard key={review.id} review={review} handleViewDish={handleViewDish} />
        ))}
      </div>
    </div>
  );
